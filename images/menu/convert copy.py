...
for color,color_code in colors.items():
    ...
    for char in utf["jp"]:
        ...
        data["code"].setdefault(key, {})
        data["code"][key]["jp"] = base_path + filename + ".png"
        ...
        subprocess.Popen(["bash", "./text_to_image.sh",char,filename,size,color_code,"jp"], cwd=cwd)
        data["code"].setdefault(key, {})
        data["code"][key]["jp_8"] = base_path + filename + ".png"
...
loc_files = ["localization.json", "localization_moves.json"]
to_stitch = []
to_remove = []

def all_processes_ended(_processes):
        ...
        for key,item in loc.items():
            for lang_code,text in item.items():
                ...
                if lang_code == "jp":
                    ...
                    data["code"].setdefault(key, {})
                    data["code"][key].setdefault(lang, {})
                    data["code"][key][lang] = base_path + outfile + ".png"
                elif lang_code == "jp_8":
                    size = "8"
                    lang = "jp"
                    ...
                    data["code"].setdefault(key, {})
                    data["code"][key].setdefault(lang, {})
                    data["code"][key][lang] = base_path + filename + ".png"
...
while len(to_stitch) > 0:
    for i,stitch in enumerate(to_stitch):
        if all_processes_ended(stitch["processes"]):
            p = subprocess.Popen(["magick"] + stitch["command"], cwd=cwd)
    ...
    for i,remove in enumerate(to_remove):
        if all_processes_ended(remove["processes"]):
            for file_name in remove["files"]:
                subprocess.Popen(["rm", file_name], cwd=cwd)
            to_remove.pop(i)
    time.sleep(.001)

#copy rogue characters
subprocess.Popen(["cp \"./rogue characters/\"* ./"], cwd=cwd, shell=True)
