...
for color,color_code in colors.items():
    ...
    for char in utf["tiny"]:
        ucode = ord(char)
        key = f"utf_{ucode}"
        size = "5"
        filename = f"{key}_tiny_{size}_{color}"
        subprocess.Popen(["bash", "./text_to_image.sh",char,filename,size,color_code,"tiny"], cwd=cwd)
        data.setdefault(key, {})
        data[key]["tiny"] = base_path + filename + ".png"
...
def create_images(loc_files, data):
            ...
            for key,item in loc.items():
                ...
                for lang_code,text in item.items():
                    ...
                    if lang_code == "jp":
                        ...
                        data.setdefault(key, {})
                        data[key].setdefault(lang_code, {})
                        data[key][lang_code] = base_path + filename + ".png"
...
data = {}
create_images(["../data/load_first.json"], data)
with open("../data/load_first_map.json", "w", encoding='utf8') as f:
    json.dump(data, f, ensure_ascii=False, indent=2, separators=(',', ': '))