...
function predict_everything(player, dummy, frames_prediction)
...
function insert_projectile(player, motion_data, predicted_hit)
...
function get_horizontal_box_overlap(a_box, ax, ay, a_flip, b_box, bx, by, b_flip)
  local a_l, b_l
...
function get_push_value(dist_from_pb_center, pushbox_overlap_range, push_value_max)
...
function predict_player_movement(p1, p1_motion_data, p1_line, p2, p2_motion_data, p2_line, index)
...
function predict_projectile_movement(projectile, mdata, line, index, ignore_flip)
  mdata[index] = copytable(mdata[index - 1])
...
function predict_frames_branching(obj, anim, frame, frames_prediction, result, include_start_frame)
  local results = {}
...
--[[       for i = 0, #fdata.frames do
        local target_hash = fdata.frames[i + 1].hash
        local n = 10
        if obj.type == "player" and fdata.infinite_loop then
          n = 8
          target_hash = string.sub(fdata.frames[i + 1].hash, n)
        end
        if frame + i + 1 <= #fdata.frames then 
          local shortened_hash = string.sub(fdata.frames[frame + i + 1].hash, 1, n)
          if shortened_hash == target_hash then
            frame_to_check = frame + i + 1
            break
          end
        end
        if frame - i + 1 >= 1 then
...
function filter_lines(player_obj, lines)
  local filtered = {}
...
function predict_frames_before_landing(player)
