...
local function typeerror(caller, narg, arg, tname) argerror(caller, narg, tname .. " expected, got " .. type(arg)) end

local function checktype(caller, narg, arg, tname) if type(arg) ~= tname then typeerror(caller, narg, arg, tname) end end
...
packers["table"] = function(buffer, tbl) return packers["_table"](buffer, tbl) end
...
local unpackers = setmetatable({}, {__index = function(t, k) error("unpack '" .. k .. "' is unimplemented") end})
m.unpackers = unpackers

local function unpack_array(c, n)
   local t = {}
   local decode = unpackers["any"]
   for i = 1, n do t[i] = decode(c) end
   return t
end
...
unpackers["nil"] = function() return nil end

unpackers["false"] = function() return false end

unpackers["true"] = function() return true end
...
unpackers["fixnum_pos"] = function(c, val) return val end
...
unpackers["fixnum_neg"] = function(c, val) return val - 0x100 end
...
unpackers["int64"] = function(c)
   ...
   if b1 < 0x80 then
      ...
      return ((((((((b1 - 0xFF) * 0x100 + (b2 - 0xFF)) * 0x100 + (b3 - 0xFF)) * 0x100 + (b4 - 0xFF)) * 0x100 +
                 (b5 - 0xFF)) * 0x100 + (b6 - 0xFF)) * 0x100 + (b7 - 0xFF)) * 0x100 + (b8 - 0xFF)) - 1
   end
end
...
unpackers["fixarray"] = function(c, val) return unpack_array(c, val % 0x10) end
...
unpackers["fixmap"] = function(c, val) return unpack_map(c, val % 0x10) end
...
function m.unpack(s)
   ...
   local cursor = {s = s, i = 1, j = #s, underflow = function(self) error "missing bytes" end}
...
function m.unpacker(src)
   if type(src) == "string" then
      local cursor = {s = src, i = 1, j = #src, underflow = function(self) error "missing bytes" end}
      return function() if cursor.i <= cursor.j then return cursor.i, unpackers["any"](cursor) end end
   elseif type(src) == "function" then
      ...
      return function()
         ...
         if cursor.i <= cursor.j then return true, unpackers["any"](cursor) end
      end
   else
...
m.Msg_Pack_Writer = {}
m.Msg_Pack_Writer.__index = m.Msg_Pack_Writer

function m.Msg_Pack_Writer.new(filename)
   local self = setmetatable({}, m.Msg_Pack_Writer)
   self.file = io.open(filename, "wb")
   if not self.file then error("Cannot open file for writing: " .. filename) end
   self.count = 0
   return self
end

function m.Msg_Pack_Writer:write(object)
   -- Pack the object
   local packed = m.pack(object)
   self.len = #packed

   -- Write 4-byte length prefix (little-endian)
   self.file:write(string.char(self.len % 256, math.floor(self.len / 256) % 256, math.floor(self.len / 65536) % 256,
                               math.floor(self.len / 16777216) % 256))
...
function m.Msg_Pack_Writer:close()
   if self.file then
      self.file:close()
      self.file = nil
   end
...
m.Msg_Pack_Reader = {}
m.Msg_Pack_Reader.__index = m.Msg_Pack_Reader

function m.Msg_Pack_Reader.new(filename)
   local self = setmetatable({}, m.Msg_Pack_Reader)
   self.file = io.open(filename, "rb")
   if not self.file then error("Cannot open file for reading: " .. filename) end
   return self
end

function m.Msg_Pack_Reader:get_length()
   -- Read 4-byte length prefix
   local len_bytes = self.file:read(4)
   if not len_bytes or #len_bytes < 4 then return end -- End of file
   -- Decode length (little-endian)
   local b1, b2, b3, b4 = string.byte(len_bytes, 1, 4)
   self.len = b1 + b2 * 256 + b3 * 65536 + b4 * 16777216
   return self.len
end

function m.Msg_Pack_Reader:read()
   self.len = self.len or self:get_length()
   if not self.len then return end
   -- Read packed data
   local packed = self.file:read(self.len)
   if not packed or #packed < self.len then
      return nil -- Incomplete data
   end

   -- Unpack and return
   self.len = nil
   return m.unpack(packed)
end

function m.Msg_Pack_Reader:read_all(callback)
   local count = 0
   while true do
      local obj = self:read()
      ...
      count = count + 1
      if callback then
         local should_continue = callback(obj, count)
...
function m.Msg_Pack_Reader:close()
   if self.file then
      self.file:close()
      self.file = nil
   end
