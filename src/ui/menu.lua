...
local open_after_round_start = false
...
local save_recording_slot_popup, load_recording_slot_popup, controller_style_menu_item, life_reset_delay_item, p1_life_reset_value_gauge_item, p2_life_reset_value_gauge_item, p1_stun_reset_value_gauge_item, p2_stun_reset_value_gauge_item, stun_reset_delay_item, load_file_name_item, p1_meter_reset_value_gauge_item, p2_meter_reset_value_gauge_item, meter_reset_delay_item, slot_weight_item, counter_attack_delay_item, recording_delay_item, recording_random_deviation_item, charge_overcharge_on_item, charge_follow_player_item, parry_follow_player_item, display_parry_compact_item, blocking_item, blocking_direction_item, hits_before_red_parry_item, parry_every_n_item, prefer_down_parry_item, hits_before_counter_attack, character_select_item, distances_settings_item, air_time_player_coloring_item, attack_range_display_max_item, attack_range_display_numbers_item, attack_bars_show_decimal_item, display_hitboxes_opacity_item, display_hitboxes_ab_item, language_item
...
local function reset_background_cache()
...
local function update_gauge_items()
   if is_initialized then
      settings.training.p1_meter_reset_value =
         math.min(settings.training.p1_meter_reset_value, gamestate.P1.max_meter_count * gamestate.P1.max_meter_gauge)
      settings.training.p2_meter_reset_value =
         math.min(settings.training.p2_meter_reset_value, gamestate.P2.max_meter_count * gamestate.P2.max_meter_gauge)
...
local function update_move_selection_items(move_selection_items, move_selection_data, move_selection_settings, dummy)
   ...
   move_selection_settings.normal_button =
      tools.bound_index(move_selection_settings.normal_button, #move_selection_data.normal_buttons)
   ...
   move_selection_settings.special_button =
      tools.bound_index(move_selection_settings.special_button, #move_selection_data.special_buttons)
...
local function update_counter_attack_items()
   if is_initialized and gamestate.is_in_match then
      ...
      if counter_attack_settings then
         update_move_selection_items(
            counter_attack_move_selection_items,
            counter_attack_move_selection_data,
            counter_attack_settings,
            training.dummy
         )
         update_move_selection_data(
            counter_attack_move_selection_items,
            counter_attack_move_selection_data,
            counter_attack_settings,
            training.dummy
...
local function save_recording_slot_to_file()
   ...
   if
      not tools.write_object_to_json_file(
         recording.recording_slots[settings.training.current_recording_slot].inputs,
         path
      )
   then
   ...
   main_menu:close_popup(save_recording_slot_popup)
end

local function load_recording_slot_from_file()
   if
      #save_recording_settings.load_file_list == 0
      or save_recording_settings.load_file_list[save_recording_settings.load_file_index] == nil
   then
   ...
   main_menu:close_popup(load_recording_slot_popup)
end

local function open_save_popup()
   save_recording_slot_popup.selected_index = 1
   main_menu:open_popup(save_recording_slot_popup)
...
local function open_load_popup()
   ...
   main_menu:open_popup(load_recording_slot_popup)
end

local function create_recording_popup()
   load_file_name_item = menu_items.List_Menu_Item:new(
      "menu_file_name",
   ...
   save_recording_slot_popup = menu_items.Menu:new(
      71,
      61,
      312,
      122, -- screen size 383,223
      {
         ...
         menu_items.Button_Menu_Item:new("menu_file_cancel", function()
            main_menu:close_popup(save_recording_slot_popup)
         end),
      },
      nil,
   ...
   load_recording_slot_popup = menu_items.Menu:new(
      71,
      61,
      312,
      122, -- screen size 383,223
      {
         load_file_name_item,
         menu_items.Button_Menu_Item:new("menu_file_load", load_recording_slot_from_file),
         menu_items.Button_Menu_Item:new("menu_file_cancel", function()
            main_menu:close_popup(load_recording_slot_popup)
         end),
      },
...
local function create_dummy_tab()
   ...
   blocking_item =
      menu_items.List_Menu_Item:new("menu_blocking", settings.training, "blocking_mode", menu_tables.blocking_mode)
   blocking_item.indent = true
   blocking_direction_item = menu_items.List_Menu_Item:new(
      "menu_blocking_direction",
      settings.training,
      "blocking_direction",
      menu_tables.blocking_direction
   )
   blocking_direction_item.indent = true
   blocking_direction_item.is_visible = function()
      return settings.training.blocking_style == 1
   end

   hits_before_red_parry_item = menu_items.Inline_Integer_Item:new("hits_before_red_parry", {
      en = { "value", " ", "menu_hits", " ", "menu_hits_before_rp_suffix" },
      jp = { "menu_hits_before_rp_prefix", " ", "value", "", "menu_hits_before_rp_suffix" },
   }, settings.training, "red_parry_hit_count", 0, 20, true, 1)
   hits_before_red_parry_item.indent = true
   hits_before_red_parry_item.is_visible = function()
      return settings.training.blocking_style == 3
   end

   parry_every_n_item = menu_items.Inline_Integer_Item:new("parry_every_n", {
      en = { "menu_parry_every_prefix", " ", "value", " ", "menu_hits" },
      jp = { "menu_parry_every_prefix", " ", "value", "", "menu_parry_every_suffix" },
   }, settings.training, "parry_every_n_count", 0, 10, true, 1)
   parry_every_n_item.indent = true
   parry_every_n_item.is_visible = function()
      return settings.training.blocking_style == 3
   end

   prefer_down_parry_item =
      menu_items.On_Off_Menu_Item:new("menu_prefer_down_parry", settings.training, "prefer_down_parry", false)
   ...
   counter_attack_move_selection_items.type_item = menu_items.List_Menu_Item:new(
      "menu_counter_attack_type",
      counter_attack_settings,
      "type",
   ...
   counter_attack_move_selection_items.motion_item = menu_items.Motion_List_Menu_Item:new(
      "menu_counter_attack_motion",
      counter_attack_settings,
      "motion",
   ...
   counter_attack_move_selection_items.motion_item.is_visible = function()
      return counter_attack_settings.type == 2
   end

   counter_attack_move_selection_items.normal_button_item = menu_items.List_Menu_Item:new(
      "menu_counter_attack_button",
      counter_attack_settings,
      "normal_button",
   ...
   counter_attack_move_selection_items.special_item = menu_items.List_Menu_Item:new(
      "menu_counter_attack_special",
      counter_attack_settings,
      "special",
   ...
   counter_attack_move_selection_items.special_item.is_visible = function()
      return counter_attack_settings.type == 3
   end

   counter_attack_move_selection_items.special_button_item = menu_items.List_Menu_Item:new(
      "menu_counter_attack_button",
      counter_attack_settings,
      "special_button",
      counter_attack_move_selection_data.special_buttons,
      1,
      update_counter_attack_items
   )
   ...
   counter_attack_move_selection_items.input_display_item = menu_items.Move_Input_Display_Menu_Item:new(
      "move_input",
   ...
   hits_before_counter_attack = menu_items.Inline_Integer_Item:new("hits_before_counter_attack", {
      en = { "value", " ", "menu_hits", " ", "menu_hits_before_ca_suffix" },
      jp = { "menu_hits_before_ca_prefix", " ", "value", "", "menu_hits_before_ca_suffix" },
   }, settings.training, "hits_before_counter_attack_count", 0, 20, true)
   hits_before_counter_attack.indent = true
   hits_before_counter_attack.is_visible = function()
      return counter_attack_settings.type ~= 1
   end

   counter_attack_delay_item = menu_items.Integer_Menu_Item:new(
      "menu_counter_attack_delay",
      settings.training,
      "counter_attack_delay",
      -40,
      40,
      false,
      0
   ...
   counter_attack_delay_item.is_visible = function()
      return counter_attack_settings.type ~= 1
   end
   ...
   return {
      ...
      entries = {
         ...
         menu_items.List_Menu_Item:new(
            "menu_blocking_style",
            settings.training,
            "blocking_style",
            menu_tables.blocking_style
         ),
         blocking_item,
         blocking_direction_item,
         hits_before_red_parry_item,
         parry_every_n_item,
         prefer_down_parry_item,
         counter_attack_move_selection_items.type_item,
         counter_attack_move_selection_items.motion_item,
         counter_attack_move_selection_items.normal_button_item,
         counter_attack_move_selection_items.special_item,
         counter_attack_move_selection_items.input_display_item,
         counter_attack_move_selection_items.special_button_item,
         counter_attack_move_selection_items.option_select_item,
...
local function create_recording_tab()
   ...
   slot_weight_item = menu_items.Integer_Menu_Item:new(
      "menu_slot_weight",
      recording.recording_slots[settings.training.current_recording_slot],
      "weight",
      0,
      100,
      false,
   ...
   recording_delay_item = menu_items.Integer_Menu_Item:new(
      "menu_replay_delay",
      recording.recording_slots[settings.training.current_recording_slot],
      "delay",
      -40,
   ...
   recording_random_deviation_item = menu_items.Integer_Menu_Item:new(
      "menu_replay_max_random_deviation",
      recording.recording_slots[settings.training.current_recording_slot],
      "random_deviation",
      0,
      600,
   ...
   return {
      ...
      entries = {
         menu_items.On_Off_Menu_Item:new(
            "menu_player_positioning",
            settings.training,
            "recording_player_positioning",
            false
         ),
         menu_items.On_Off_Menu_Item:new(
            "menu_dummy_positioning",
            settings.training,
            "recording_dummy_positioning",
            false
         ),
         menu_items.On_Off_Menu_Item:new(
            "menu_auto_crop_first_frames",
            settings.training,
            "auto_crop_recording_start",
            false
         ),
         menu_items.On_Off_Menu_Item:new(
            "menu_auto_crop_last_frames",
            settings.training,
            "auto_crop_recording_end",
            false
         ),
         menu_items.List_Menu_Item:new(
            "menu_replay_mode",
         ...
         menu_items.Integer_Menu_Item:new(
            "menu_slot",
            settings.training,
            "current_recording_slot",
            1,
            recording.recording_slot_count,
            true,
            1,
            1,
            10,
         ...
         menu_items.Label_Menu_Item:new(
            "recording_slot_frames",
            { "value", " ", "menu_frames" },
            recording.current_recording_slot_frames,
            "frames",
            false,
            true
         ),
         ...
         menu_items.Button_Menu_Item:new("menu_clear_all_slots", function()
            recording.clear_all_slots()
            recording.update_current_recording_slot_frames()
         end),
         menu_items.Button_Menu_Item:new("menu_save_slot_to_file", open_save_popup),
...
local function create_display_tab()
   controller_style_menu_item = menu_items.Controller_Style_Item:new(
      "menu_controller_style",
      settings.training,
      "controller_style",
   ...
   attack_bars_show_decimal_item = menu_items.On_Off_Menu_Item:new(
      "menu_show_decimal",
      settings.training,
   ...
   attack_bars_show_decimal_item.is_visible = function()
      return settings.training.display_attack_bars > 1
   end

   display_hitboxes_ab_item =
      menu_items.On_Off_Menu_Item:new("menu_display_hitboxes_ab", settings.training, "display_hitboxes_ab", false)
   display_hitboxes_ab_item.indent = true
   display_hitboxes_ab_item.is_visible = function()
      return settings.training.display_hitboxes > 1
   end
   display_hitboxes_ab_item.on_change = reset_background_cache

   display_hitboxes_opacity_item = menu_items.Integer_Menu_Item:new(
      "menu_display_hitboxes_opacity",
      settings.training,
      "display_hitboxes_opacity",
      5,
      100,
      false,
      100,
   ...
   display_hitboxes_opacity_item.is_visible = function()
      return settings.training.display_hitboxes > 1
   end
   display_hitboxes_opacity_item.on_change = reset_background_cache

   distances_settings_item = menu_items.Button_Menu_Item:new("menu_settings", function()
      main_menu.main_menu_selected_index = modules_tab_index
      main_menu.sub_menu_selected_index = 1
      local distances_module_index = 1
      for i, name in ipairs(modules_names) do
         if name == "distances_distances" then
            distances_module_index = i
            break
         end
      end
      settings.training.modules_index = distances_module_index
      local distances_module = modules.get_module("distances")
      if not distances_module.is_enabled then
         modules.toggle(distances_module)
      end
      update_active_modules_page()
   end)
   distances_settings_item.indent = true
   distances_settings_item.is_visible = function()
      return settings.training.display_distances
   end

   air_time_player_coloring_item = menu_items.On_Off_Menu_Item:new(
      "menu_display_air_time_player_coloring",
      settings.training,
      "display_air_time_player_coloring",
      false,
      reset_background_cache
   )
   air_time_player_coloring_item.indent = true
   air_time_player_coloring_item.is_visible = function()
      return settings.training.display_air_time
   end

   charge_overcharge_on_item = menu_items.On_Off_Menu_Item:new(
      "menu_display_overcharge",
      settings.training,
   ...
   charge_overcharge_on_item.is_visible = function()
      return settings.training.display_charge
   end

   charge_follow_player_item = menu_items.On_Off_Menu_Item:new(
      "menu_follow_player",
      settings.training,
   ...
   charge_follow_player_item.is_visible = function()
      return settings.training.display_charge
   end

   parry_follow_player_item = menu_items.On_Off_Menu_Item:new(
      "menu_follow_player",
      settings.training,
      "parry_follow_player",
      false,
   ...
   parry_follow_player_item.is_visible = function()
      return settings.training.display_parry
   end

   display_parry_compact_item = menu_items.On_Off_Menu_Item:new(
      "menu_display_parry_compact",
      settings.training,
   ...
   display_parry_compact_item.is_visible = function()
      return settings.training.display_parry
   end

   attack_range_display_max_item = menu_items.Integer_Menu_Item:new(
      "menu_attack_range_display_max_attacks",
      settings.training,
      "attack_range_display_max_attacks",
      1,
      3,
      true,
      1
   ...
   attack_range_display_max_item.is_visible = function()
      return settings.training.display_attack_range ~= 1
   end
   attack_range_display_max_item.on_change = reset_background_cache

   attack_range_display_numbers_item = menu_items.On_Off_Menu_Item:new(
      "menu_attack_range_display_show_numbers",
      settings.training,
   ...
   attack_range_display_numbers_item.is_visible = function()
      return settings.training.display_attack_range ~= 1
   end

   language_item = menu_items.List_Menu_Item:new(
      "menu_language",
      settings.training,
      "language",
      menu_tables.language,
      1,
      function()
         main_menu:update_dimensions_of_all_items()
         main_menu:update_page_position()
         modules.update_language()
   ...
   return {
      ...
      entries = {
         ...
         menu_items.List_Menu_Item:new(
            "menu_display_input_history",
            settings.training,
            "display_input_history",
            menu_tables.display_input_history_mode,
            1,
            reset_background_cache
         ),
         menu_items.On_Off_Menu_Item:new(
            "menu_display_gauge_numbers",
            settings.training,
            "display_gauges",
            false,
            reset_background_cache
         ...
         menu_items.List_Menu_Item:new(
            "menu_display_attack_bars",
            settings.training,
            "display_attack_bars",
            menu_tables.display_attack_bars_mode,
            3,
            reset_background_cache
         ),
         attack_bars_show_decimal_item,
         menu_items.List_Menu_Item:new(
            "menu_display_frame_advantage",
            settings.training,
            "display_frame_advantage",
            menu_tables.display_frame_advantage_mode,
            1,
            reset_background_cache
         ),
         menu_items.List_Menu_Item:new(
            "menu_display_hitboxes",
            settings.training,
            "display_hitboxes",
         ...
         menu_items.On_Off_Menu_Item:new(
            ...
            false,
            function()
               local dist_module = modules.get_module("distances")
               dist_module.init()
               reset_background_cache()
            end
         ),
         distances_settings_item,
         menu_items.On_Off_Menu_Item:new(
            "menu_display_stun_timer",
            settings.training,
            "display_stun_timer",
            true,
            reset_background_cache
         ...
         menu_items.On_Off_Menu_Item:new(
            "menu_display_blocking_direction",
            settings.training,
            "display_blocking_direction",
            false,
            reset_background_cache
         ),
         menu_items.On_Off_Menu_Item:new(
            "menu_display_red_parry_miss",
            settings.training,
            "display_red_parry_miss",
            false,
            reset_background_cache
         ),
         menu_items.List_Menu_Item:new(
            "menu_display_attack_range",
            settings.training,
         ...
         menu_items.List_Menu_Item:new("menu_theme", settings.training, "theme", menu_tables.theme_names, 1, function()
            colors.set_theme(settings.theme)
            main_menu:reset_background_cache()
         end),
         language_item,
      },
...
local function create_rules_tab()
   character_select_item =
      menu_items.Button_Menu_Item:new("menu_character_select", character_select.start_character_select_sequence)

   local function get_life_bar_color(self)
   ...
   p1_life_reset_value_gauge_item = menu_items.Gauge_Menu_Item:new(
      "menu_p1_life_reset_value",
      settings.training,
      "p1_life_reset_value",
      1,
      1,
   ...
   p2_life_reset_value_gauge_item = menu_items.Gauge_Menu_Item:new(
   ...
   life_reset_delay_item =
      menu_items.Integer_Menu_Item:new("menu_reset_delay", settings.training, "life_reset_delay", 1, 100, false, 20)

   p1_life_reset_value_gauge_item.indent = true
   p2_life_reset_value_gauge_item.indent = true
   life_reset_delay_item.indent = true

   p1_life_reset_value_gauge_item.is_visible = function()
   ...
   p1_life_reset_value_gauge_item.color_function = get_life_bar_color
   p2_life_reset_value_gauge_item.color_function = get_life_bar_color

   p1_life_reset_value_gauge_item.show_numbers = true
   p2_life_reset_value_gauge_item.show_numbers = true

   p1_stun_reset_value_gauge_item = menu_items.Gauge_Menu_Item:new(
      "menu_p1_stun_reset_value",
      settings.training,
      "p1_stun_reset_value",
      1,
      1,
   ...
   p2_stun_reset_value_gauge_item = menu_items.Gauge_Menu_Item:new(
   ...
   stun_reset_delay_item =
      menu_items.Integer_Menu_Item:new("menu_reset_delay", settings.training, "stun_reset_delay", 1, 100, false, 20)

   p1_stun_reset_value_gauge_item.indent = true
   p2_stun_reset_value_gauge_item.indent = true
   stun_reset_delay_item.indent = true

   p1_stun_reset_value_gauge_item.is_visible = function()
   ...
   p1_stun_reset_value_gauge_item.show_numbers = true
   p2_stun_reset_value_gauge_item.show_numbers = true

   p1_meter_reset_value_gauge_item = menu_items.Gauge_Menu_Item:new(
   ...
   meter_reset_delay_item =
      menu_items.Integer_Menu_Item:new("menu_reset_delay", settings.training, "meter_reset_delay", 1, 100, false, 20)

   p1_meter_reset_value_gauge_item.indent = true
   p2_meter_reset_value_gauge_item.indent = true
   meter_reset_delay_item.indent = true

   p1_meter_reset_value_gauge_item.is_visible = function()
   ...
   p1_meter_reset_value_gauge_item.show_numbers = true
   p2_meter_reset_value_gauge_item.show_numbers = true
   ...
   return {
      ...
      entries = {
         ...
         menu_items.List_Menu_Item:new(
            "menu_life_refill_mode",
            settings.training,
            "life_mode",
            menu_tables.life_mode,
            4,
            update_gauge_items()
         ),
         ...
         menu_items.List_Menu_Item:new(
            "menu_stun_refill_mode",
            settings.training,
            "stun_mode",
            menu_tables.stun_mode,
            3,
            update_gauge_items()
         ),
         ...
         menu_items.List_Menu_Item:new(
            "menu_meter_refill_mode",
            settings.training,
            "meter_mode",
            menu_tables.meter_mode,
            5,
            update_gauge_items()
         ),
         ...
         menu_items.Integer_Menu_Item:new("menu_music_volume", settings.training, "music_volume", 0, 200, false, 0, 5),
      },
   }
end
...
local function create_training_tab()
   training_mode_item =
      menu_items.List_Menu_Item:new("menu_mode", settings.training, "training_mode_index", training_mode_names, 1)
   ...
   training_tab_page =
      menu_items.Page_Navigation_Menu_Item:new("menu_title_training", training_pages, training_mode_item)
...
local function create_modules_tab()
   ...
   modules_item.legend = function()
      if modules.extra_modules[settings.training.modules_index].is_enabled then
         return "legend_lp_disable"
      end
...
local function create_debug_tab()
   return {
      ...
      entries = {
         ...
         menu_items.On_Off_Menu_Item:new(
            "menu_debug_variables_display",
            debug_settings,
            "show_debug_variables_display",
            false
         ),
         menu_items.On_Off_Menu_Item:new(
            "menu_debug_frames_display",
            debug_settings,
            "show_debug_frames_display",
            false
         ),
         ...
         menu_items.Button_Menu_Item:new("menu_record_frame_data", function()
...
local save_settings_request = { should_save = false, countdown = 0 }

local function queue_save_settings()
   save_settings_request.should_save = true
   save_settings_request.countdown = 2
end

local function update_save_settings()
   if save_settings_request.should_save then
      if save_settings_request.countdown == 0 then
         recording.backup_recordings()
         settings.save_training_data()
         save_settings_request.should_save = false
      end
      save_settings_request.countdown = save_settings_request.countdown - 1
   end
end
...
local function create_menu()
   ...
   if debug_settings.developer_mode then
      menu_tabs[#menu_tabs + 1] = create_debug_tab()
   end
...
local function open_menu()
   if not disable_opening then
      is_open = true
      open_after_round_start = false
      update_menu_items()
      main_menu:menu_stack_push(main_menu)
      if gamestate.is_in_match_playable and not disable_freeze then
   ...
   if is_initialized then
      ...
      if is_open then
         ...
         local input = {
            ...
            left = tools.check_input_down_autofire(
               gamestate.P1,
               "left",
               horizontal_autofire_rate,
               horizontal_autofire_time
            ),
            right = tools.check_input_down_autofire(
               gamestate.P1,
               "right",
               horizontal_autofire_rate,
               horizontal_autofire_time
...
local function load_menu(target_menu)
   update_menu_items = target_menu.update_menu_items
   create_menu = target_menu.create_menu
   update = target_menu.update
   open_menu = target_menu.open_menu
   close_menu = target_menu.close_menu
   reset_background_cache = target_menu.reset_background_cache

   menu_module.update_menu_items = update_menu_items
   menu_module.create_menu = create_menu
   menu_module.update = update
   menu_module.open_menu = open_menu
   menu_module.close_menu = close_menu
   menu_module.reset_background_cache = reset_background_cache
end
...
setmetatable(menu_module, {
   __index = function(_, key)
      ...
      elseif key == "open_after_round_start" then
         return open_after_round_start
