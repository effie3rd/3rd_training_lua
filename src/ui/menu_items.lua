...
function Textfield_Menu_Item:draw(x, y, selected)
   ...
   if self.is_in_edition then
      color = text.button_activated_color
...
function Textfield_Menu_Item:validate()
   if not self.is_in_edition then
      self:sync_from_var()
      if #self.content < self.max_length then table.insert(self.content, 1) end
...
function On_Off_Menu_Item:left() self.object[self.property_name] = not self.object[self.property_name] end

function On_Off_Menu_Item:right() self.object[self.property_name] = not self.object[self.property_name] end

function On_Off_Menu_Item:reset() self.object[self.property_name] = self.default_value end
...
function List_Menu_Item:calc_dimensions()
   self.width, self.height =
       get_text_dimensions_multiple({self.name, ":  ", self.list[self.object[self.property_name]]})
end
...
function Check_Box_Grid_Item:draw(x, y, selected)
   ...
   for _, name in ipairs(self.list) do
      local w, h = get_text_dimensions(name)
   ...
   render_text_multiple(x + offset_x, y + offset_y, text_table, nil, nil, color)

   local tx, ty = get_text_dimensions_multiple(text_table)
   offset_x = offset_x + tx

   self.row_height = ty + 2

   local sel_index = (self.selected_row - 1) * self.cols + self.selected_col
   local base_color = text.default_color
   if not self:is_enabled() then base_color = text.disabled_color end
   ...
   for row = 1, self.rows do
      ...
      for col = 1, self.cols do
         ...
         if self.list[index] then
            ...
            local w, h = get_text_dimensions(self.list[index])
...
function Check_Box_Grid_Item:calc_dimensions()
   ...
   while j <= #self.list do
      ...
      for i = 1, self.cols do
         local tw, th = get_text_dimensions(self.list[j + i - 1])
...
function Hits_Before_Menu_Item:draw(x, y, selected)
   ...
   if settings.language == "en" then
      if self.object[self.property_name] == 1 then hits_text = "hit" end
      render_text(x + offset_x, y, hits_text, nil, nil, color)
      w, h = get_text_dimensions(hits_text)
      offset_x = offset_x + w + 1
   end
   if self.suffix ~= "" then render_text(x + offset_x, y, self.suffix, nil, nil, color) end
...
function Header_Menu_Item:draw(x, y, state)
   ...
   elseif state == "inactive" then
      color = text.inactive_color
   end

   render_text(x, y, self.name, nil, nil, color)
end

function Header_Menu_Item:calc_dimensions() self.width, self.height = get_text_dimensions(self.name) end
...
function Footer_Menu_Item:draw(x, y, state)
   ...
   render_text(x, y, self.name, settings.language, nil, color)
end

function Footer_Menu_Item:calc_dimensions() self.width, self.height = get_text_dimensions(self.name) end
...
function Multitab_Menu:calc_dimensions()
   for i = 1, #self.content do
      ...
      for j = 1, #self.content[i].entries do
         if self.content[i].entries[j].calc_dimensions then self.content[i].entries[j]:calc_dimensions() end
      end
   end
end
...
function Multitab_Menu:menu_stack_push(menu) table.insert(self.menu_stack, menu) end
...
function Multitab_Menu:update_dimensions() for i, menu in ipairs(self.menu_stack) do menu:calc_dimensions() end end
...
function Multitab_Menu:draw()
   ...
   for i = 1, #self.content do
      ...
      menu_y = self.top + self.y_padding * 2 + self.content[i].header.height
   end
   for pad = 15, 35 do gui.drawline(self.left + pad, menu_y - 1, self.right - pad, menu_y - 1, colors.menu.divider) end
   ...
   if not self.is_main_menu_selected then
      if self:current_entry().legend then
         render_text(menu_x, legend_y, self:current_entry():legend(), nil, nil, text.inactive_color)
      end
   end

   local scroll_up = self.content[self.main_menu_selected_index].top_entry_index > 1
   if scroll_down or scroll_up then
      render_text(self.right - w - self.x_padding, legend_y, "legend_hp_scroll", nil, nil, text.inactive_color)

      local scroll_up_y_pos = menu_y + h / 2 - 3
      local scroll_down_y_pos = menu_y + (y_offset - self.menu_item_spacing - h) + h / 2 - 2
