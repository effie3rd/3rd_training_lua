...
local function update_selected_player_color()
   if settings.modules.extra_settings.p1_coloring == 1 then
      if player_coloring[1].is_enabled then
   ...
   if settings.modules.extra_settings.p2_coloring == 1 then
      if player_coloring[2].is_enabled then
...
local function update_player_coloring()
   if gamestate.is_in_match then
      if gamestate.is_in_curtain then
...
local function idle_time_display()
   ...
   for _, player in pairs(players) do
      ...
      local idle_text = Pools.draw:alloc()
      idle_text[1], idle_text[2], idle_text[3] = "hud_idle", ": ", idle_time
      local w, h = draw.get_text_dimensions_multiple(idle_text, font)
      local x, y = draw.game_to_screen_space(player.pos_x, player.pos_y)
      draw.render_text_multiple(x - w / 2, y + 4, idle_text, font, colors.idle_time)
   end
end
...
local function disable_player_coloring()
   ...
   player_coloring[2].should_change = true
   if
      memory.readdword(gamestate.P1.addresses.palette) ~= player_coloring[1].color
      or memory.readdword(gamestate.P2.addresses.palette) ~= player_coloring[2].color
   then
...
local function create_menu()
   ...
   extra_settings_menu.p1_coloring_item = menu_items.List_Menu_Item:new(
      "extra_settings_p1_coloring",
      settings.modules.extra_settings,
      "p1_coloring",
      extra_settings_menu_tables.player_coloring,
      1,
      update_selected_player_color
   )
   ...
   extra_settings_menu.p2_coloring_item = menu_items.List_Menu_Item:new(
      "extra_settings_p2_coloring",
      settings.modules.extra_settings,
      "p2_coloring",
      extra_settings_menu_tables.player_coloring,
      1,
   ...
   extra_settings_menu.idle_time_item = menu_items.List_Menu_Item:new(
      "extra_settings_display_idle_time",
      settings.modules.extra_settings,
      "display_idle_time",
   ...
   extra_settings_menu.version_item =
      menu_items.Label_Menu_Item:new("menu_version", { "menu_version", " ", game_data.script_version })
   return {
      name = "extra_settings_extra_settings",
      entries = {
         extra_settings_menu.p1_coloring_item,
         extra_settings_menu.p2_coloring_item,
         extra_settings_menu.idle_time_item,
         extra_settings_menu.version_item,
      },
   }
end
...
local function init()
   is_enabled = settings.modules.extra_settings.is_enabled
   if is_enabled then
      update_selected_player_color()
      update_idle_time_display()
