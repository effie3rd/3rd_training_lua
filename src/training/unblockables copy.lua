...
local match_start_state = savestate.create("data/"..rom_name.."/savestates/unblockables_match_start.fs")
local followup_state = savestate.create("data/"..rom_name.."/savestates/unblockables_followup.fs")

local unblockable_data
local unblockable_type
local unblockable_followup
...
local function continue_from_savestate()
  Register_After_Load_State(
    function ()
      player = gamestate.P1
      dummy = gamestate.P2
      local active_followups = {}
      for i = 1, #unblockable_data.followups do
        if settings.training.modes.unblockables.followups[i] then
    ...
    end, nil, after_load_delay)
  savestate.load(followup_state)
end

local function start(sel_player, sel_dummy, sel_type, sel_followup)
  Register_After_Load_State(function () is_active = true end)
  if settings.training.modes.unblockables.savestate_player == sel_player
  and settings.training.modes.unblockables.savestate_dummy == sel_dummy
  and settings.training.modes.unblockables.savestate_type == sel_type
  then
    continue_from_savestate()
  else
    inputs.block_input(1, "all")
    inputs.block_input(2, "all")
    unblockable_type = sel_type
    unblockable_followup = sel_followup
...
local function start_character_select()
  ...
  unblockable_data = unblockables_tables.get_unblockables_data(settings.training.modes.unblockables.character, settings.training.modes.unblockables.type)
  print(settings.training.modes.unblockables.type)
...
local function update()
  if is_active then
    if gamestate.is_in_match then
      if state == states.SETUP_MATCH_START
      or state == states.SETUP
      or state == states.WAIT_FOR_SETUP
      then
        inputs.block_input(1, "all")
        inputs.block_input(2, "all")
      end
      if state == states.SETUP or state == states.WAIT_FOR_SETUP then
        hud.show_please_wait_display(true)
      end
      if state == states.SETUP_MATCH_START and gamestate.has_match_just_started then
        savestate.save(match_start_state)
        settings.training.modes.unblockables.character = player.char_str
        require("src.ui.menu").open_menu()
        is_active = false
      elseif state == states.SETUP then
        player = gamestate.P1
        dummy = gamestate.P2
        unblockable_data = unblockables_tables.get_unblockables_data(player.char_str, settings.training.modes.unblockables.type)
      ...
      elseif state == states.WAIT_FOR_SETUP then
        if advanced_control.all_commands_queued(dummy)
        ...
        then
          Queue_Command(gamestate.frame_number + 1, function ()
            ...
            settings.training.modes.unblockables.savestate_player = gamestate.P1.char_str
            settings.training.modes.unblockables.savestate_dummy = gamestate.P2.char_str
            settings.training.modes.unblockables.savestate_type = settings.training.modes.unblockables.type
      ...
      elseif state == states.RUNNING then
        if (player.standing_state >= 3 )
        or player.has_just_hit_ground
        or dummy.has_just_hit_ground
        then
          state = states.END
...
local function selection_matches_savestate(sel_player, sel_dummy)
  --cxbxs
  if settings.training.modes.unblockables.savestate_player == sel_player
  and settings.training.modes.unblockables.savestate_dummy == sel_dummy
  then
...
local unblockables = {
  ...
  reset = reset,
  update = update,
  selection_matches_savestate = selection_matches_savestate,
