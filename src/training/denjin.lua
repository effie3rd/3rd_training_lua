...
local function start(sel_player, sel_dummy, sel_type, sel_followup)
   ...
   if settings.special_training.unblockables.savestate_player == sel_player and
       settings.special_training.unblockables.savestate_dummy == sel_dummy and
       settings.special_training.unblockables.savestate_type == sel_type then
...
local function start_character_select()
   ...
   print(settings.special_training.unblockables.type)
   ...
   if is_active then
      if gamestate.is_in_match then
         ...
         elseif state == states.RUNNING then
            if (player.standing_state >= 3) or player.has_just_hit_ground or dummy.has_just_hit_ground then
               state = states.END
...
local function selection_matches_savestate(sel_player, sel_dummy)
   -- cxbxs
   if settings.special_training.unblockables.savestate_player == sel_player and
       settings.special_training.unblockables.savestate_dummy == sel_dummy then return true end
   return false
end

local function process_gesture(gesture) if gesture == "single_tap" then start() end end
