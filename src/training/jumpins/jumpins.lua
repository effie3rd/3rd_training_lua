...
local framedata_meta = require("src.data.framedata_meta")
...
local jumpins_tables = require("src.training.jumpins.jumpins_tables")
...
local jumpins_menu = {}
local jumpins_edit_menu = {}
local jumpins_edit_items = {}
local jumpins_edit_move_selection_items = {}
local jumpins_edit_move_selection_data = {}
local jumpins_edit_jump_index = 1

local jumpins_menu_tables = {
   fixed_point_dynamic = {"menu_fixed_point", "menu_dynamic"},
   replay_mode = {"menu_replay_random", "menu_replay_ordered"},
   offset_mode = {"menu_fixed_point", "menu_range_random", "menu_range_ordered", "menu_range_endpoints"}
}

local jumpins_edit_settings = {}
local should_update_while_menu_is_open = false
local show_notifications_on_menu_close = false
...
local function draw_jump_arc(jump_arc)
   ...
   local function get_color(index)
      ...
      elseif mode == MAIN_MODES.RUN then
...
local function jumpins_display()
   ...
   if mode == MAIN_MODES.EDIT then draw_player_distances(jumpins_dummy) end
...
local function execute_jump(player, first_jump_name, second_jump_name, second_jump_delay, attack_name, attack_delay,
   ...
   if followup and followup.type ~= 1 then
      ...
      elseif followup.type == 3 then
         ...
         if tools.table_contains(require("src.data.move_data").kara_command_throws, followup_data.name) then
            local base_name = {
...
local function load_settings() jumpins_settings = settings.modules.jumpins.characters[jumpins_dummy.char_str] end
...
local function load_all_jumps()
   jumpins_settings = settings.modules.jumpins.characters[jumpins_dummy.char_str]
   if not jumpins_settings then jumpins_settings = jumpins_tables.create_settings(jumpins_dummy) end
   jumps = {}
...
local function begin_edit(jump_settings)
...
local function start()
   training.set_module_control_by_name(module_name, "player")
   load_character_data()
   inputs.unblock_input(1)
   inputs.unblock_input(2)
   load_settings()
...
local function update()
   if is_active then
      ...
      elseif mode == MAIN_MODES.RUN then
         ...
         if show_notifications_on_menu_close then
            if not menu.is_open then
               ...
               show_notifications_on_menu_close = false
            end
         end
...
local function update_jumpins_settings()
   if gamestate.is_in_match then
      load_character_data()
      jumpins_edit_settings = settings.modules.jumpins.characters[jumpins_dummy.char_str]
      if not jumpins_edit_settings then
         settings.modules.jumpins.characters[jumpins_dummy.char_str] = jumpins_tables.create_settings(jumpins_dummy)
         jumpins_edit_settings = settings.modules.jumpins.characters[jumpins_dummy.char_str]
      end
...
local function update_jumpins_range_items()
   jumpins_edit_items.player_reset_position_item.points = player_position_range
   jumpins_edit_items.player_reset_position_item.point_index = {value = 1}
   jumpins_edit_items.player_reset_position_item.range = player_position_bounds
   jumpins_edit_items.player_reset_position_item.mode = {value = 1}

   jumpins_edit_items.dummy_reset_offset_item.points = dummy_offset_range
   jumpins_edit_items.dummy_reset_offset_item.point_index = dummy_offset_edit_index
   jumpins_edit_items.dummy_reset_offset_item.range = dummy_offset_bounds
   jumpins_edit_items.dummy_reset_offset_item.mode = dummy_offset_edit_mode

   jumpins_edit_items.attack_delay_item.points = attack_delay_range
   jumpins_edit_items.attack_delay_item.point_index = attack_delay_edit_index
   jumpins_edit_items.attack_delay_item.range = attack_delay_bounds
   jumpins_edit_items.attack_delay_item.mode = attack_delay_edit_mode

   jumpins_edit_items.second_jump_delay_item.points = second_jump_delay_range
   jumpins_edit_items.second_jump_delay_item.point_index = {value = 1}
   jumpins_edit_items.second_jump_delay_item.range = second_jump_delay_bounds
   jumpins_edit_items.second_jump_delay_item.mode = {value = 1}
end
...
local function update_menu()
   if gamestate.is_in_match then
      ...
      menu.update_move_selection_items(jumpins_edit_move_selection_items, jumpins_edit_move_selection_data,
                                       current_jump_settings.followup, jumpins_dummy)
      menu.update_move_selection_data(jumpins_edit_move_selection_items, jumpins_edit_move_selection_data,
                                      current_jump_settings.followup, jumpins_dummy)

      jumpins_tables.update_character(jumpins_dummy.char_str)

      jumpins_edit_items.jump_type_item.object = current_jump_settings
      jumpins_edit_items.jump_type_item.list = jumpins_tables.get_menu_jump_names()

      load_jump(current_jump_settings)

      update_jumpins_range_items()

      jumpins_edit_items.second_jump_type_item.object = current_jump_settings
      jumpins_edit_items.second_jump_type_item.list = jumpins_tables.get_menu_second_jump_names()
      jumpins_edit_items.second_jump_delay_item.object = current_jump_settings
      jumpins_edit_items.attack_type_item.object = current_jump_settings
      jumpins_edit_items.attack_type_item.list = jumpins_tables.get_menu_attack_names()
      ...
      jumpins_menu.player_position_mode_item.object = jumpins_edit_settings
      jumpins_menu.dummy_offset_mode_item.object = jumpins_edit_settings
      jumpins_menu.attack_delay_mode_item.object = jumpins_edit_settings
      jumpins_menu.show_jump_arc_item.object = jumpins_edit_settings
      jumpins_menu.show_jump_info_item.object = jumpins_edit_settings
      jumpins_menu.automatic_replay_item.object = jumpins_edit_settings
      jumpins_menu.next_attack_delay_item.object = jumpins_edit_settings
...
local function create_jumpins_edit_menu()
   jumpins_edit_settings = settings.modules.jumpins.characters["alex"]
   current_jump_settings = jumpins_edit_settings.jumps[1]
   jumpins_edit_move_selection_data = {
   ...
   jumpins_edit_items = {}
   jumpins_edit_items.jump_type_item = menu_items.List_Menu_Item:new("menu_jump", current_jump_settings, "jump_name",
                                                                     jumpins_tables.get_menu_jump_names(), 1)
   jumpins_edit_items.jump_type_item.on_change = function() update_selected_jump() end

   jumpins_edit_items.player_reset_position_item = menu_items.Slider_Menu_Item:new("menu_player_position", 40,
                                                                                   {400, 400}, {200, 800})
   jumpins_edit_items.player_reset_position_item.disable_mode_switch = true
   jumpins_edit_items.player_reset_position_item.legend_text = ""
   jumpins_edit_items.player_reset_position_item.left = function() move_player_left() end
   jumpins_edit_items.player_reset_position_item.right = function() move_player_right() end
   jumpins_edit_items.player_reset_position_item.is_enabled = is_jump_selected
   jumpins_edit_items.player_reset_position_item.is_unselectable = no_jump_selected

   jumpins_edit_items.dummy_reset_offset_item = menu_items.Slider_Menu_Item:new("menu_dummy_offset", 40, {80, 80},
                                                                                {-100, 100})
   jumpins_edit_items.dummy_reset_offset_item.left = function() move_dummy_left() end
   jumpins_edit_items.dummy_reset_offset_item.right = function() move_dummy_right() end
   jumpins_edit_items.dummy_reset_offset_item.is_enabled = is_jump_selected
   jumpins_edit_items.dummy_reset_offset_item.is_unselectable = no_jump_selected

   jumpins_edit_items.second_jump_type_item = menu_items.List_Menu_Item:new("menu_second_jump", current_jump_settings,
                                                                            "second_jump_name",
                                                                            jumpins_tables.get_menu_second_jump_names(),
                                                                            1)
   jumpins_edit_items.second_jump_type_item.is_visible = function()
      return #jumpins_edit_items.second_jump_type_item.list > 0
   end
   jumpins_edit_items.second_jump_type_item.is_enabled = is_jump_selected
   jumpins_edit_items.second_jump_type_item.is_unselectable = no_jump_selected
   jumpins_edit_items.second_jump_type_item.on_change = function() update_selected_jump() end

   jumpins_edit_items.second_jump_delay_item = menu_items.Slider_Menu_Item:new("menu_second_jump_delay", 40, {8, 8},
                                                                               {8, 40})
   jumpins_edit_items.second_jump_delay_item.disable_mode_switch = true
   jumpins_edit_items.second_jump_delay_item.legend_text = ""
   jumpins_edit_items.second_jump_delay_item.on_change = function() update_selected_jump() end

   jumpins_edit_items.second_jump_delay_item.is_visible = function()
      return jumpins_edit_items.second_jump_type_item.is_visible() and current_jump_settings.second_jump_name ~= 1
   end
   jumpins_edit_items.second_jump_delay_item.is_enabled = is_jump_selected
   jumpins_edit_items.second_jump_delay_item.is_unselectable = no_jump_selected

   jumpins_edit_items.attack_type_item = menu_items.List_Menu_Item:new("menu_attack", current_jump_settings,
                                                                       "attack_name",
                                                                       jumpins_tables.get_menu_attack_names(), 1)
   jumpins_edit_items.attack_type_item.is_enabled = is_jump_selected
   jumpins_edit_items.attack_type_item.is_unselectable = no_jump_selected
   jumpins_edit_items.attack_type_item.on_change = update_menu

   jumpins_edit_items.attack_delay_item = menu_items.Slider_Menu_Item:new("menu_attack_delay", 40, {5, 5}, {5, 100})

   jumpins_edit_items.attack_delay_item.is_enabled = is_jump_selected
   jumpins_edit_items.attack_delay_item.is_unselectable = no_jump_selected
   ...
   jumpins_edit_items.followup_delay_item = menu_items.Integer_Menu_Item:new("menu_followup_delay",
                                                                             current_jump_settings, "followup_delay",
                                                                             -40, 40, false, 0)
   jumpins_edit_items.followup_delay_item.is_enabled = is_jump_selected
   jumpins_edit_items.followup_delay_item.is_unselectable = no_jump_selected

   jumpins_edit_items.status_item = menu_items.Label_Menu_Item:new("status", {
   ...
   return menu_items.Menu:new(0, 0, 150, 150, {
      jumpins_edit_items.jump_type_item, jumpins_edit_items.player_reset_position_item,
      jumpins_edit_items.dummy_reset_offset_item, jumpins_edit_items.second_jump_type_item,
      jumpins_edit_items.second_jump_delay_item, jumpins_edit_items.attack_type_item,
      jumpins_edit_items.attack_delay_item, jumpins_edit_move_selection_items.type_item,
      ...
      jumpins_edit_items.followup_delay_item
   }, nil, true, jumpins_edit_items.status_item, true)
end
...
local function create_menu()
   ...
   jumpins_edit_menu.on_open = function()
   ...
   jumpins_edit_menu.on_close = function()
      ...
      should_update_while_menu_is_open = false
      menu.allow_update_while_open = false
      if menu.is_open then modes.start(jumpins) end
   end
   ...
   jumpins_menu.start_item.is_unselectable = function() return not jumpins_menu.start_item.is_enabled() end

   jumpins_menu.start_edit_item = menu_items.Button_Menu_Item:new("menu_settings", function()
      modes.set_active(jumpins)
   ...
   jumpins_menu.jump_replay_mode_item = menu_items.List_Menu_Item:new("menu_jump_replay_mode", jumpins_edit_settings,
                                                                      "jump_replay_mode",
                                                                      jumpins_menu_tables.replay_mode)
   jumpins_menu.player_position_mode_item = menu_items.List_Menu_Item:new("menu_player_position_mode",
                                                                          jumpins_edit_settings, "player_position_mode",
                                                                          jumpins_menu_tables.fixed_point_dynamic)
   jumpins_menu.dummy_offset_mode_item = menu_items.List_Menu_Item:new("menu_dummy_offset_mode", jumpins_edit_settings,
                                                                       "dummy_offset_mode",
                                                                       jumpins_menu_tables.offset_mode)
   jumpins_menu.attack_delay_mode_item = menu_items.List_Menu_Item:new("menu_attack_delay_mode", jumpins_edit_settings,
                                                                       "attack_delay_mode",
                                                                       jumpins_menu_tables.offset_mode)
   jumpins_menu.show_jump_arc_item = menu_items.On_Off_Menu_Item:new("menu_show_jump_arc", jumpins_edit_settings,
                                                                     "show_jump_arc", false)
   jumpins_menu.show_jump_info_item = menu_items.On_Off_Menu_Item:new("menu_show_jump_info", jumpins_edit_settings,
                                                                      "show_jump_info", false)
   jumpins_menu.automatic_replay_item = menu_items.On_Off_Menu_Item:new("menu_automatic_replay", jumpins_edit_settings,
                                                                        "automatic_replay", true)
   jumpins_menu.next_attack_delay_item = menu_items.Integer_Menu_Item:new("menu_next_attack_delay",
                                                                          jumpins_edit_settings, "next_attack_delay", 0,
                                                                          120, false, 0)

   return {
      name = "training_jumpins",
      entries = {
         jumpins_menu.start_item, character_select_and_open_menu_item, jumpins_menu.start_edit_item,
         jumpins_menu.jump_replay_mode_item, jumpins_menu.player_position_mode_item,
         jumpins_menu.dummy_offset_mode_item, jumpins_menu.attack_delay_mode_item, jumpins_menu.show_jump_arc_item,
         jumpins_menu.show_jump_info_item, jumpins_menu.automatic_replay_item, jumpins_menu.next_attack_delay_item
      }
   }
end
