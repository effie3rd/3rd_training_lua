...
function Setup:new(name, type)
   local obj = {name = name, type = type or Action_Type.ATTACK, timeout = 3 * 60, should_lock_positions = true}
...
function Setup:label() return "menu_" .. self.name end

local Followup = {}
Followup.__index = Followup

function Followup:new(name, type)
   local obj = {name = name, type = type or Action_Type.ACTION, timeout = 3 * 60}

   setmetatable(obj, self)
   return obj
end

function Followup:init() end
...
function Followup:label() return "menu_" .. self.name end
