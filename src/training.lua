...
local function write_player_vars(player_obj)
  ...
  if player_obj.id == 1 then
    wanted_meter = training_settings.p1_meter
  elseif player_obj.id == 2 then
    wanted_meter = training_settings.p2_meter
  end
  ...
  if gamestate.is_in_match and not IsMenuOpen and not player_obj.is_in_timed_sa then
    ...
    elseif training_settings.meter_mode == 2 then
      if player_obj.is_idle and player_obj.idle_time > training_settings.meter_refill_delay then
  ...
  if training_settings.infinite_sa_time and player_obj.is_in_timed_sa then
  ...
  elseif training_settings.stun_mode == 3 then
    if gamestate.is_in_match and not IsMenuOpen and player_obj.is_idle then
      ...
      if player_obj.id == 1 then
        ...
        wanted_stun = training_settings.p2_stun_reset_value
      end
      ...
      elseif player_obj.is_idle and player_obj.idle_time > training_settings.stun_reset_delay then
...
local function write_game_vars()
  ...
  set_music_volume(training_settings.music_volume)
end
