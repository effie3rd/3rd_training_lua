...
local function queue_input_sequence(player, sequence, offset, allow_blocking)
  offset = offset or 0
  ...
  if player.pending_input_sequence.current_frame >= 1 then
    ...
    for i = 1, #current_frame_input do
      ...
      elseif current_frame_input[i] == "360" then
        ...
        memory.writebyte(player.addresses.kaiten_2, 15)
        ...
        --   memory.writebyte(player.addresses.kaiten_completed_360, 56)
        -- end
...
local function clear_directional_input(input)
  if input == nil then
    return
  end
...
local function clear_p1_buttons(input)
  if input == nil then
    return
  end
  input["P1 Weak Punch"] = false
...
local function create_counter_attack_input_sequence(counter_attack_data)
  --recording
  if counter_attack_data.ca_type == 5 then
    return nil
  end
  ...
  if counter_attack_data.ca_type == 2 then
    ...
    if stick == "kara_throw" then
      sequence = {{"LP","LK"}}
      table.insert(sequence, 1, counter_attack_data.inputs)
  ...
  elseif counter_attack_data.ca_type == 3 then
    sequence = counter_attack_data.inputs
