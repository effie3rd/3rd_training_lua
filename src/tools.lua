...
function flip_to_sign(n)
  return n == 0 and -1 or (n == 1 and 1)
end
...
function clamp(_number, min, max)
  return math.max(math.min(_number, max), min)
end
...
function write_object_to_json_file(_object, _file_path, _indent)
  ...
  local _str = ""
  if _indent then
    _str = json.encode(_object, { indent = true })
  else
    _str = json.encode(_object)
  end
...
function combine_arrays(_a, _b)
  local combined = {}
  local n = 1
  for _, v in ipairs(_a) do
    combined[n], n = v, n + 1
  end
  for _, v in ipairs(_b) do
    combined[n], n = v, n + 1
  end
  return combined
end
...
function float_to_byte(_n)
  ...
  local _error = 0x01 / 256
  if _mantissa >= _error then
    local _min = 0x0
    local _max = 0xFF
    local _guess = bit.rshift(_min + _max, 1)
    repeat
      if math.abs(_guess / 256 - _mantissa) < _error then
        return _guess
      elseif _guess / 256 < _mantissa then
        _min = _guess
        _guess = bit.rshift(_guess + _max, 1)
      elseif _guess / 256 > _mantissa then
        _max = _guess
        _guess = bit.rshift(_guess + _min, 1)
      end
    until _guess == _min or _guess == _max
    return _guess
  end
  return 0
end
...
function format_box(_box)
  return {type = convert_box_types[_box[1]],
          bottom = _box[2],
          height = _box[3],
          left = _box[4],
          width = _box[5]}
