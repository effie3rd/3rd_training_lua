...
local function save_controller_settings()
   controller_cache = { training.P1_controller.name, training.P2_controller.name }
end
local function save_training_settings()
   for _, key in ipairs(settings_to_save) do
      settings_cache[key] = settings.training[key]
   end
end

local function restore_controller_settings()
   training.set_controllers_by_name(controller_cache[1], controller_cache[2])
end
local function restore_training_settings()
   for key, value in pairs(settings_cache) do
      settings.training[key] = value
   end
end
...
local function init()
   ...
   modes = {}
   local training_modules = require("src.modules").training_modules
   local extra_modules = require("src.modules").extra_modules
   for _, module in ipairs(training_modules) do
      if module.start then
         modes[#modes + 1] = module
      end
   end
...
local function stop()
   ...
   active_mode.stop()
   active_mode.is_mode_active = false
   restore_controller_settings()
   restore_training_settings()
   if active_mode.end_mode then
      active_mode.end_mode()
   end
   active_mode = nil
end
