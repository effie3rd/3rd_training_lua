...
local action_data = {}
local should_record_action_data = true
local state = ""
local setup = false
local normals = {}
...
local final_props = {
   ...
   "max_hits", "cooldown", "self_chain", "exceptions", "landing_anim", "landing_height"
} --  "advantage"
local final_frame_props = {
   "hash", "boxes", "movement", "velocity", "acceleration", "loop", "next_anim", "optional_anim", "wakeup",
   "bypass_freeze", "projectile", "ignore_motion", "clear_motion"
}
local function save_frame_data()
   for _, char in ipairs(frame_data_keys) do
      if fd.frame_data[char].should_save then
         fd.frame_data[char].should_save = nil
         local fdata = tools.deepcopy(fd.frame_data[char])
            ...
            if type(data) == "table" and id ~= "wakeups" then
               for k, v in pairs(data) do
                  if k == "name" then
                     ...
                     elseif v == "air_recovery" then
                        fdata.air_recovery = id
...
local action_data_file_ext = "_actiondata.json"

local function save_action_data()
   for _, char in ipairs(frame_data_keys) do
      local adata = action_data[char]
      if adata then
         local file_path = framedata_path .. "@" .. char .. action_data_file_ext
         if not tools.write_object_to_json_file(adata, file_path, true) then
            print(string.format("Error: Failed to write action data to \"%s\"", file_path))
         else
            print(string.format("Saved action data to \"%s\"", file_path))
         end
      end
   end
end
...
local function is_hit_frame(frame)
   if frame.boxes then
      for _, box in pairs(frame.boxes) do
         local type = tools.get_box_type_string(box[1])
...
local function serialize_framedata()
   ...
   local total_size = 0
   for _, char in ipairs(frame_data_keys) do
      for id, data in pairs(fd.frame_data[char]) do
...
local function process_motion_data(anim_list)
   ...
   for i = 1, #anim_list do
      anim_list[i].frames[1].first_frame = true
      if anim_list[i].is_landing_recovery then anim_list[i].frames[1].first_landing_frame = true end
   ...
   for i = #all_frames, 1, -1 do
      if all_frames[i].raw_movement and all_frames[i].raw_velocity and all_frames[i].raw_acceleration then
         if i - 1 >= 1 and all_frames[i - 1].raw_movement and all_frames[i - 1].raw_velocity and
            ...
            if (uses_velocity[i] and not discard_motion_data[i]) or (not uses_velocity[i] and all_frames[i].first_frame) then
            ...
            if all_frames[i].first_landing_frame then
               all_frames[i].movement[2] = 0
               all_frames[i].velocity[1] = 0
               all_frames[i].velocity[2] = 0
               all_frames[i].acceleration[1] = 0
               all_frames[i].acceleration[2] = 0
               all_frames[i].clear_motion = true
            end
...
local function record_action_data(obj, action_name)
   local key = obj.projectile_type and "projectiles" or obj.char_str
   if not action_data[key] then action_data[key] = {} end
   local start_address = memory.readdword(obj.base + memory_addresses.offsets.action_address)
   local line_size = memory.readbyte(obj.base + memory_addresses.offsets.action_line_size)
   local data = {name = action_name, line_size = line_size, lines = {}}
   -- print(start_address, line_size, action_name)
   local i = start_address
   repeat
      data.lines[#data.lines + 1] = {}
      local current_line = data.lines[#data.lines]
      for j = 0, line_size - 1 do
         current_line[#current_line + 1] = string.format("%08X", memory.readdword(i + j * 4))
      end
      i = i + line_size * 4
      local command = string.sub(current_line[1], 1, 4)
   until command == "0001"
   -- print(data.name, string.format("%08X", i - line_size * 4), data.line_size, #data.lines)
   action_data[key][string.format("%08X", start_address)] = data
end
...
local function new_recording(player, projectiles, name)
   ...
   if should_record_action_data then record_action_data(player, name) end
...
local function end_recording(player, projectiles, name)
   ...
   if (fd.frame_data[player.char_str] == nil) then fd.frame_data[player.char_str] = {} end
   fd.frame_data[player.char_str].should_save = true
   ...
   for i = 1, #current_recording_anim_list do
      ...
      local fdata = fd.frame_data[player.char_str][id]
      ...
      if fdata == nil or rec.recording_options.clear_frame_data then
         fd.frame_data[player.char_str][id] = current_recording_anim_list[i]
      else
   ...
   for id, _ in pairs(ids) do
      fd.frame_data[player.char_str][id].frames = handle_loops(fd.frame_data[player.char_str][id].frames)
   end

   for id, _ in pairs(ids) do
      local anim = fd.frame_data[player.char_str][id]
      local frames = fd.frame_data[player.char_str][id].frames
      ...
      for i = 1, #frames do
         if frames[i].next_anim then
            for k, na in pairs(frames[i].next_anim) do
               if na.hash then
                  local index = index_of_hash(fd.frame_data[player.char_str][na.id].frames, na.hash)
      ...
      if rec.recording_options.is_air_recovery then
         local _, jump_back_fdata = fd.find_frame_data_by_name(player.char_str, "jump_back")
         local _, jump_neutral_fdata = fd.find_frame_data_by_name(player.char_str, "jump_neutral")
         local _, jump_forward_fdata = fd.find_frame_data_by_name(player.char_str, "jump_forward")
         for i = 1, #frames do
            if not tools.has_boxes(frames[i].boxes, {"vulernability", "ext_vulnerability"}) then
               local frame_id = string.sub(frames[i].hash, 1, 4)
               local boxes_found = false
               if jump_back_fdata then
                  for __, jump_frame in ipairs(jump_back_fdata.frames) do
                     local jump_frame_id = string.sub(jump_frame.hash, 1, 4)
                     if jump_frame_id == frame_id then
                        frames[i].boxes = jump_frame.boxes
                        boxes_found = true
                     end
                  end
               end
               if not boxes_found and jump_neutral_fdata then
                  for __, jump_frame in ipairs(jump_neutral_fdata.frames) do
                     local jump_frame_id = string.sub(jump_frame.hash, 1, 4)
                     if jump_frame_id == frame_id then
                        frames[i].boxes = jump_frame.boxes
                        boxes_found = true
               ...
               if not boxes_found and jump_forward_fdata then
                  for __, jump_frame in ipairs(jump_forward_fdata.frames) do
                     local jump_frame_id = string.sub(jump_frame.hash, 1, 4)
                     if jump_frame_id == frame_id then
                        frames[i].boxes = jump_frame.boxes
                        boxes_found = true
   ...
   if rec.recording then
      ...
      if player.remaining_freeze_frames == 0 or bypassing_freeze then
         ...
         if (frame == 0 and not player.is_attacking and player.posture == 0 and player.pos_y == 0 and
            ...
            additional_props.is_landing_recovery = true
            ...
            if current_recording_anim_list[#current_recording_anim_list - 1] then
               current_recording_anim_list[#current_recording_anim_list - 1].landing_anim = player.animation
            end
            if rec.recording_options.recording_landing then
               if current_recording_anim_list[#current_recording_anim_list - 1] then
      ...
      for _, obj in pairs(projectiles) do
         if obj.emitter_id == player.id then
            ...
            if i == 0 then
               ...
               if should_record_action_data then record_action_data(obj, name) end
            else
            ...
            if data.type == type then
               ...
               if obj.remaining_freeze_frames == 0 then
                  ...
                  for __, box in ipairs(obj.boxes) do
                     local box_type = tools.get_box_type_string(box[1])
                     if (box_type == "attack") or (box_type == "throw") then
      ...
      for key, proj_list in pairs(current_recording_proj_list) do
         ...
         if not tools.table_contains_deep(projectiles, obj) then
            ...
            for i, proj in ipairs(proj_list) do
               ...
               local fdata = fd.frame_data["projectiles"][id]
               if fd.frame_data["projectiles"][id] == nil or rec.overwrite then
                  fd.frame_data["projectiles"][id] = proj
                  fd.frame_data["projectiles"].should_save = true
               else
                  local j = 1
                  while j <= #new_frames do
                     if new_frames[j].discard then
            ...
            for id, _ in pairs(ids) do
               ...
               local p_index = 1
               local anim = fd.frame_data["projectiles"][id]
               ...
               for i = 1, #frames do
                  ...
                  if frames[i].next_anim then
                     for k, na in pairs(frames[i].next_anim) do
                        if na.hash then
                           local index = index_of_hash(fd.frame_data["projectiles"][na.id].frames, na.hash)
                           if index == 0 then index = 1 end
                           frames[i].next_anim[k] = {na.id, index - 1}
                        end
                     end
...
local movement_list = {
   ...
   "jump_neutral", "jump_back", "sjump_forward", "sjump_neutral", "sjump_back", "air_recovery", "air_jump_forward",
   "air_jump_neutral", "air_jump_back", "air_dash", "block_high_proximity", "block_low_proximity",
   "block_high_air_proximity", "block_high_air", "parry_high", "parry_low", "parry_air"
}
...
local function record_movement(player)
   ...
   if gamestate.is_in_match and debug_settings.recording_framedata then
      ...
      elseif state == "wait_for_initial_anim" and player.has_animation_just_changed then
         ...
         elseif name == "air_recovery" then
            if player.action == 393216 then new_recording(player, {}, name) end
      ...
      if state == "queue_move" then
         ...
         if i_movement_list <= #movement_list then
            ...
            elseif name == "air_recovery" then
               allow_dummy_movement = true
               rec.recording_options.is_air_recovery = true
               sequence = {{"up"}, {"up"}, {"up"}, {}, {}, {}, {}}
               local contact_dist = fd.get_contact_distance(player) - 40
               if player.char_str == "twelve" then
                  contact_dist = 50
               elseif player.char_str == "yun" then
                  contact_dist = contact_dist - 20
               end
               Queue_Command(gamestate.frame_number + clear_jump_after, write_memory.clear_motion_data, {player})
               Queue_Command(gamestate.frame_number + clear_jump_after, write_memory.write_pos,
                             {player, dummy.pos_x - contact_dist, 110})
               Queue_Command(gamestate.frame_number + clear_jump_after,
                             function() rec.recording_options.discard_motion_data = true end)
               local dummy_jump_height = 110
               if player.char_str == "necro" then dummy_jump_height = 120 end
               inputs.queue_input_sequence(dummy, sequence)
               Queue_Command(gamestate.frame_number + clear_jump_after, write_memory.write_pos_y,
                             {dummy, dummy_jump_height})
               Queue_Command(gamestate.frame_number + clear_jump_after, inputs.queue_input_sequence, {dummy, {{"MP"}}})
...
local function record_wakeups(player)
   ...
   if gamestate.is_in_match and debug_settings.recording_framedata then
      ...
      if state == "queue_move" then
         if i_wakeups <= #wakeups_list then
            ...
            if dummy.char_str ~= current_attack.character then
               ...
               Call_After_Load_State(character_select.force_select_character, {player.id, player.char_str, 1, "LP"})
               Call_After_Load_State(character_select.force_select_character,
      ...
      if setup then
         ...
         if should_tap_down and current_attack.quick then inputs.input[player.prefix .. " Down"] = true end
   ...
   while i <= #moves do
      ...
      if moves[i].move_type == "special" then
         ...
         elseif moves[i].name == "hayate" then
            ...
            table.insert(moves, i + 1, move)
         elseif moves[i].name == "spin_sides" then
            local move = tools.deepcopy(moves[i])
            move.name = "spin_sides_1"
            table.insert(moves, i + 1, move)
...
local function record_attacks(player, projectiles)
   if gamestate.is_in_match and debug_settings.recording_framedata then
      ...
      if dummy.char_str ~= "urien" then
         state = "wait_for_match_start"
         Call_After_Load_State(character_select.force_select_character, {player.id, player.char_str, 1, "LP"})
         Call_After_Load_State(character_select.force_select_character, {dummy.id, "urien", 1, "MP"})
         character_select.start_character_select_sequence()
         return
      end
      ...
      if state == "queue_move" then
         ...
         elseif rec.current_attack_category.name == "specials" then
            ...
            if player.char_str == "elena" then
               ...
               if base_name == "spin_sides_1" then
                  if not (button == "EXK") then
                     rec.i_attacks = rec.i_attacks + 1
                     state = "queue_move"
                     return
                  end
               ...
               if current_attack.is_projectile then
                  ...
                  if rec.current_projectile then
                     for _, box in pairs(rec.current_projectile.boxes) do
                        if tools.get_box_type_string(box[1]) == "attack" then
...
local function record_all_characters(player, projectiles)
   if debug_settings.recording_framedata then
      ...
      settings.training.tech_throws_mode = 1
      settings.training.fast_wakeup_mode = 1
      settings.training.mash_inputs_mode = 1
      settings.training.infinite_sa_time = false
      settings.training.universal_cancel = false
      settings.training.infinite_projectiles = false
      settings.training.infinite_juggle = false
      settings.training.auto_parrying = 1
      local dummy = player.other
      if record_char_state == "start" then
         fd.frame_data["projectiles"] = {}
      ...
      elseif record_char_state == "new_character" then
         if i_characters <= end_character then
            ...
            rec.block_max_hits = 0
            rec.recording_options.hit_type = "miss"
            Call_After_Load_State(character_select.force_select_character, {player.id, char, 1, "LP"})
            Call_After_Load_State(character_select.force_select_character, {dummy.id, "urien", 1, "MP"})
            character_select.start_character_select_sequence()
