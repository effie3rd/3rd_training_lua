...
local function idle_just_began(player)
   return player.just_became_idle
      or player.has_just_started_jump
      or ((advantage[player.id] and advantage[player.id].hitbox_start_frame) and player.has_just_thrown)
end
...
local function finish_advantage(player, advantage_obj)
   ...
   if advantage_obj.opponent_reference_frame and advantage_obj.player_reference_frame then
      ...
      advantage_obj.just_finished = true
   end
end

local function update(menu_is_open, menu_allow_update_while_open)
   ...
   for i, player in ipairs(gamestate.player_objects) do
      if advantage[player.id] and advantage[player.id].active then
         if advantage[player.id].state == advantage_states.START then
            if not player.is_idle then
               advantage[player.id].state = advantage_states.RUNNING
            end
         elseif advantage[player.id].state == advantage_states.RUNNING then
            ...
            if not advantage[player.id].hitbox_start_frame then
               ...
               if
                  ...
                  or player.other.has_just_received_connection
                  or tools.has_boxes(player.boxes, tools.BOXES.ATTACK_AND_THROW)
                     and not (player.char_str == "yang" and player.animation == "c79c" and player.animation_frame < 7)
               then
                  has_hitboxes = true
               else
               ...
               if has_hitboxes then
                  ...
                  advantage[player.id].startup = gamestate.frame_number
                     - advantage[player.id].start_frame
            ...
            if advantage[player.id].hitbox_start_frame and not advantage[player.id].hitbox_end_frame then
               if not advantage[player.id].projectile then
                  if not tools.has_boxes(player.boxes, tools.BOXES.ATTACK_AND_THROW) then
            ...
            if not advantage[player.id].connect_frame then
               if player.has_just_connected or player.other.has_just_received_connection then
                  ...
                  advantage[player.id].hit_frame = gamestate.frame_number
                     - advantage[player.id].start_frame
                     - advantage[player.id].freeze_frames
                     + 1
               end
            ...
            advantage[player.id].duration = advantage[player.id].end_frame
               - advantage[player.id].start_frame
               - advantage[player.id].freeze_frames

            if
               idle_just_began(player)
               or player.has_just_landed
               or (
                  not gamestate.is_ground_state(player.other, player.other.standing_state)
                  and (player.action == 2 or player.action == 3)
               )
            then
            ...
            if
               (player.is_idle or idle_just_began(player)) and (player.other.is_idle or idle_just_began(player.other))
            then
         ...
         if menu_is_open and not menu_allow_update_while_open then
      ...
      elseif advantage[player.id] and advantage[player.id].just_finished then
